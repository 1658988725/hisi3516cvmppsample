...
HI_S32 SAMPLE_COMM_VENC_SaveH264(FILE* fpH264File, VENC_STREAM_S *pstStream)
{
    ...
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
        ...
        //fflush(fpH264File);
		//printf("g_rtspSession:%d\n",g_rtspSession);
		if(pstStream->pstPack[i].DataType.enH264EType == H264E_NALU_ISLICE ||
			pstStream->pstPack[i].DataType.enH264EType == H264E_NALU_PSLICE ||
			pstStream->pstPack[i].DataType.enH264EType == H264E_NALU_SPS ||
			pstStream->pstPack[i].DataType.enH264EType == H264E_NALU_PPS)
		
		cdr_rtsp_send_frame(0,pstStream->pstPack[i].pu8Addr+pstStream->pstPack[i].u32Offset,\
