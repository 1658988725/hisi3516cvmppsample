    	...
    	while(AbortSignal == 0)
    	{
			...
			{	
				if(rtsp[pMs->conn_index]->rtp_tcp_mode==RTP_TCP)
				{
					...
					{					
						send_data_ret = build_rtp_tcp_nalu(pbuf->data,pbuf->size , pMs->conn_index,0,audio_encode_type,0,(pbuf->time/100.0)*TIME_STAMP_S);//(pbuf->time*TIME_STAMP_S)&0x000000003fffffffll);
					}					                      
				}
				else
				{
